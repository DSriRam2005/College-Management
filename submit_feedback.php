...
if(isset($_GET['ajax']) && $_GET['ajax']==1){
    ...
    if($val === 'all'){
        $sql = "SELECT htno, $col FROM FEEDBACK WHERE $col IS NOT NULL AND TRIM($col) != ''";
    } else {
        $sql = "SELECT htno, $col FROM FEEDBACK WHERE $col = '$val'";
    }
...
function getCounts($conn, $column) {
    $sql = "SELECT $column as val, COUNT(*) as cnt FROM FEEDBACK GROUP BY $column";
    $res = $conn->query($sql);
    $data = [];
    while($row = $res->fetch_assoc()){
...
function getAverage($conn, $column){
    $sql = "SELECT AVG($column) as avg_val FROM FEEDBACK";
    $res = $conn->query($sql)->fetch_assoc();
    return is_null($res['avg_val']) ? '-' : round($res['avg_val'],2);
}

$totalFeedbacks = $conn->query("SELECT COUNT(*) as c FROM FEEDBACK")->fetch_assoc()['c'];
...
<?php foreach($openText as $col=>$label):
    $res = $conn->query("SELECT COUNT(*) as c FROM FEEDBACK WHERE $col IS NOT NULL AND TRIM($col) != ''");
