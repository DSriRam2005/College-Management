...
if (isset($_POST['search'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (strtoupper(trim($student['prog'])) !== 'B.TECH' || $student['year'] != 22) {
...
if (isset($_POST['save'])) {
    ...
    $updateStudent = $conn->prepare("
        UPDATE STUDENTS 
        SET firstname=?, middlename=?, lastname=?, phone=? 
        WHERE htno=?
    ");
    $updateStudent->bind_param("sssss", $firstname, $middlename, $lastname, $phone, $htno);
...
<?php if (strtoupper(trim($student['prog']))==='B.TECH' && $student['year']==22 && $student['gen']==='F'): ?>
<label><input type="checkbox" name="infosys_verified" value="1" <?= (!empty($placement['infosys_verified']))?'checked disabled':'' ?>> Verified Email for Infosys Drive</label>
<?php endif; ?>
