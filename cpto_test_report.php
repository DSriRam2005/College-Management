...
-- ✅ FILTER BY LOGIN USER'S CLASSID
WHERE TRIM(UPPER(s.classid)) = TRIM(UPPER(?))

GROUP BY s.htno, s.name, s.teamid

-- ✅ ORDER BY TEAMID NUMERICALLY
ORDER BY CAST(SUBSTRING_INDEX(s.teamid, '_', -1) AS UNSIGNED), s.teamid, s.htno;
");

$query->bind_param("s", $classid);
$query->execute();
...
<h2>TEST REPORT (CLASS: <?= $classid ?>)</h2>
